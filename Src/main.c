...
uint32_t ADC1_VREF = 0;


uint32_t USART_UE_MASK = 0x00001000;		  // UE - 13 bit
uint32_t USART_TCIE_MASK = 0x00000020;		  // TCIE - 6 bit
uint32_t USART_TC_MASK = 0x00000020;   	      // TC - 6 bit
uint32_t USART_TXE_MASK = 0x00000040;   	  // TXE - 7 bit
int USART_VAL;
/* USER CODE END PM */
...
int* fill_parabola(const int n){
	...
	int dV(uint16_t Vref, uint16_t ADC_bits_amount){
		if(ADC_bits_amount == 12)
			return (int)((float)Vref*1000/4096);
		return 0;
	}

	void ADC_controller_with_pregenerated_sinusoid_PWM(__IO uint32_t* PWM_CCRx, __IO uint32_t* ADC_CCRx,__IO uint32_t* T_CCRx, const uint32_t max_pulse_val)
	{
		int i = 0;
		int n = 360;
		int* arr = fill_sine(n, max_pulse_val);
		// const float f = T((float)ADC1_JDR2/dv);
		// start adc (ADON)
		...
		/*
		Vrefint - напряжение, поданное на adc1->J3 из документации
		dV = (float)Vd / ADC1_VREF  - цена деления
		op = dV * 4096 - опора
...
//void DMA_ADC_controller(__IO uint32_t* PWM_CCRx, __IO uint32_t* ADC_CCRx,const uint32_t max_pulse_val){
...
//// page 15
//	int16_t* dma_val = (int16_t*)calloc(1, sizeof(int16_t));
//	*dma_val = 0;
//	ADC1->CR2 |= ADC1_ADON_MASK;
//	ADC1->CR1 |= ADC1_JEOCIE_MASK;
//	ADC1->CR2 |= ADC1_CR2_JSWSTART_MASK;
//
//	DMA2_Stream0->CR &= (~1);    // clr prev dma settings
//	while(DMA2_Stream0->CR & 1); // wait for end of proc
//	DMA2->LISR = 0; // clr LISR
//	DMA2->HISR = 0;	// clr HISR
//	DMA2_Stream0->PAR = &(ADC1->DR); // PTR to source
//	DMA2_Stream0->M0AR = dma_val; // PTR to target
//	DMA2_Stream0->NDTR = 1; // amount of items
//	DMA2_Stream0->CR &= (~0x0E000000); // CHSEL
//	DMA2_Stream0->CR &= (~0x00000010); // ? DMEIE
//	// DMA2_Stream0->CR &= (~0x00000000); // priority
//
//
//	while(1)
//	{
...
//}
//
//void dma_snippet(__IO uint32_t* PWM_CCRx, __IO uint32_t* ADC_CCRx,__IO uint32_t* T_CCRx, const uint32_t max_pulse_val)
//{
//	// Program M0AR with QUADSPI data register address */
//	DMA2_Stream0->M0AR = (uint32_t)ADC1_JDR1;
//	/* Program PAR with Buffer address */
//	DMA2_Stream0->PAR = (uint32_t)&u32Buffer[0];
//	/* Write number of data items to transfer */
//	DMA2_Stream0->NDTR = 0x100;
...
void USART_controller(){

	// UE - usart enable  (13)
	USART2->CR1 |= USART_UE_MASK;
	// TCIE - enable interrupt (6)
	USART2->CR1 |= USART_TCIE_MASK;
	int num = 0xFFFFFFFF;
	while(1)
	{
		if((USART2->SR & USART_TC_MASK) || (USART2->SR & USART_TXE_MASK))
		{
			USART2->DR = num;
...
/* Configure the peripherals common clocks */
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /*
    ADC_controller_with_pregenerated_sinusoid_PWM(&TIM4->CCR4, &TIM4->CCR1, &TIM4->CCR2, 10000);

    */

    USART_controller();
    // DMA_ADC_controller(&TIM4->CCR4, &TIM4->CCR1, &TIM4->CCR2, 10000);
    /* USER CODE BEGIN 3 */
  /* USER CODE END 3 */
}
